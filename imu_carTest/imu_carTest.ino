...
//THE CAR TEST MODIFICATIONS SIMPLY CHANGE THE BASE LINE ACCELERATION VALUES BECAUSE
//THE CAR CANNOT ACCELERATE TO A G.
...
//constants
const float noiseBuffer = 0.75; //arbitrary value for now
...
float flightStageDecision(float zAccel) {
  ...
  if (zAccel >= 0 - noiseBuffer && zAccel <= 0 + noiseBuffer) { //force of gravity //buffer is included due to possibly noise in LSM readings
    flightStage = 0; //pre-launch, do not activate at all
  } else if (zAccel < -noiseBuffer) { // force of gravity plus engine thrust
    flightStage = 1; // burn
